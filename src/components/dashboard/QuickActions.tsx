    ...
    >
      {actions.map((action) => (
        ...
        >
          <Link
            href={action.href}
            className="block p-4 bg-white rounded-xl border border-gray-100 hover:border-coral/20 hover:shadow-sm transition-all group"
          >
            ...
            <p className="text-xs text-navy/40 mt-0.5">{action.description}</p>
          </Link>
        </motion.div>
      ))}
    </motion.div>
